...
`
  ...
  describe('project level tsconfig.json', () => {
    ...
    it('should not prune stale project references from projects included in projectsToKeepStaleReferences', async () => {
      ...
      writeJson(tree, 'packages/b/tsconfig.json', {
        ...
        references: [
          ...
          // but project `b` is included in `projectsToKeepStaleReferences`, so we don't
          // prune any of the existing references
          { path: '../c' },
        ],
      });
      tree.write('some-path/dir/tsconfig.json', '{}');
